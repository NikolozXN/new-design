...
/** Rough horizontal scroll distance before the track is measured — prevents a zero-height frame. */
function estimateDistance() {
  if (typeof window === "undefined") return 3200;
  const intro = 420;
  const panels = FEATURES.length * 432;
  return Math.max(0, intro + panels - window.innerWidth + 32);
}
...
/** Phones + reduced motion: normal responsive grid. */
...
/** Desktop: horizontal scroll-pinned showcase — the signature landing animation. */
    ...
    >
      <div className="sticky top-0 flex h-[100dvh] items-center overflow-hidden pb-12 pt-24 sm:pt-28">
        <motion.div
          ref={trackRef}
          style={{ x }}
          className="flex items-stretch gap-6 px-6 will-change-transform lg:px-8"
        >
          <IntroPanel pinned />
          {FEATURES.map((f) => (
        ...
        <div className="pointer-events-none absolute inset-x-8 bottom-[12%] sm:inset-x-12">
          <div className="h-px bg-border" />
          <motion.div
            style={{ width: fillWidth }}
            className="absolute left-0 top-0 h-0.5 rounded-full bg-gradient-to-r from-primary to-accent shadow-[0_0_12px_2px_var(--primary)]"
          />
        </div>
      </div>
    </section>
...
export function Features() {
  ...
  return (
    <div id="features">
      {/* Mobile: lightweight grid */}
      <div className="md:hidden">
        <FeaturesGrid />
      </div>
      {/* Desktop: scroll-pinned horizontal journey (always mounted — no late swap) */}
